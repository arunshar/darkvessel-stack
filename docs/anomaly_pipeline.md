...
> Status: this is a DESIGN document for the intended anomaly pipeline. Of the steps below, only the Pi-DPM conditional-diffusion head (Step 4) and the TGARD haversine / gap geometry (Step 2) are implemented. traj-CLIP alignment (Step 1), STAGD + DRM (Step 3), the calibrated logistic combination (Step 5), and the worked end-to-end example are NOT implemented: there is no traj-CLIP encoder, no `dark_distortion` module, no detection / classification head feeding this pipeline, and no fitted weights. The numbers in the worked example below are ILLUSTRATIVE, not measured. The Gradio Space prints a seeded pseudo-score, not the output of this pipeline.

The seven RS heads are intended to tell us *where* and *what* the vessel is; the anomaly pipeline is intended to tell us *whether it should be there*. This is where the dissertation contributions plug in.
...
## Step 1: traj-CLIP alignment (from trajprompt) [PLANNED, not implemented]
...
## Step 2: TGARD gap detection (Sharma, Ghosh, Shekhar, ACM TIST 2024) [geometry implemented]
...
## Step 3: STAGD + DRM (Sharma, Ghosh, Shekhar, ACM TIST 2024) [PLANNED, not implemented]
...
## Step 4: Pi-DPM physics-informed reconstruction (Sharma et al., ACM SIGSPATIAL GeoAnomalies 2025) [IMPLEMENTED: heads/anomaly.py + pidpm/]
...
## Step 5: dark-vessel probability and reasoning trace [PLANNED, not implemented]
...
## End-to-end example [ILLUSTRATIVE, fabricated for exposition; not a measured run]

The trace below is a hand-written illustration of what the intended output would look like. No detection, MMSI, score, or probability in it was produced by running code; the pipeline that would produce it is not fully implemented.
